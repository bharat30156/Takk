...
console.log(111, formData)
  ...
  if(formData.id!=-1) {
    fetch("http://localhost:8080/api/initiative/"+formData.id, {
      method: "PUT",
    ...
    .then((res) => {
        if (res.ok) {
          ...
          navigate('/InitiativeView',{state:{originalformData:formData}})
    ...
    .catch((error) => console.error("Error:", error));
      ...
      fetch('http://localhost:8080/api/initiative/create', {
        method: 'POST',    
        body: JSON.stringify(formData),    
        headers: {
          'Content-Type': 'application/json'
