import styles from "./ViewInitiativePage.module.css";

const ViewInitiativePage = () => {
  return (
    <div className={styles.viewInitiativePage}>
      <div className={styles.takkLogoYellow2Parent}>
        <img
          className={styles.takkLogoYellow2}
          alt=""
          src="/takk-logo--yellow-21@2x.png"
        />
        <div className={styles.frameWrapper}>
          <div className={styles.frameChild} />
        </div>
        <div className={styles.searchParent}>
          <div className={styles.search}>Search</div>
          <img className={styles.image6Icon} alt="" src="/image-6@2x.png" />
        </div>
        <a className={styles.nameOfUser}>Name of User</a>
        <img className={styles.frameItem} alt="" src="/ellipse-4@2x.png" />
        <div className={styles.ellipseParent}>
          <img className={styles.groupChild} alt="" src="/ellipse-51.svg" />
          <div className={styles.div}>3</div>
        </div>
        <div className={styles.getFundraisingParent}>
          <a className={styles.getFundraising}>Get Fundraising</a>
          <a className={styles.donate}>Donate</a>
          <a className={styles.community}>Community</a>
        </div>
        <div className={styles.frameInner} />
      </div>
      <div className={styles.image62Parent}>
        <img className={styles.image62Icon} alt="" src="/image-62@2x.png" />
        <img className={styles.image63Icon} alt="" src="/image-63@2x.png" />
        <img className={styles.image51Icon} alt="" src="/image-51@2x.png" />
        <div className={styles.lineDiv} />
        <div className={styles.image9Parent}>
          <div className={styles.image9} />
          <img className={styles.ellipseIcon} alt="" src="/ellipse-8.svg" />
          <div className={styles.raised}>
            <b>£1,500</b>
            <span> raised</span>
          </div>
          <div className={styles.target}>
            <b>£3,000</b>
            <span>
              <span className={styles.span}>{` `}</span>
              <span>target</span>
            </span>
          </div>
          <b className={styles.b}>50%</b>
          <a className={styles.rectangleParent}>
            <div className={styles.groupItem} />
            <a className={styles.donateNow}>Donate Now</a>
          </a>
          <div className={styles.youDonated343}>You donated £343</div>
          <img className={styles.image127Icon} alt="" src="/image-127@2x.png" />
          <img className={styles.image128Icon} alt="" src="/image-128@2x.png" />
          <a className={styles.image129} />
        </div>
        <div className={styles.frameChild1} />
        <div
          className={styles.descriptionAboutCommunity}
        >{`Description about community Lorem ipsum dolor sit, amet consectetur adipisicing elit. Cumque similique eaque delectus unde commodi assumenda, rerum, et doloribus nobis qui eius voluptatum vitae suscipit quos quod? Description about community post Lorem ipsum dolor sit, amet consectetur adipisicing elit. `}</div>
        <button className={styles.rectangleGroup}>
          <div className={styles.groupInner} />
          <a className={styles.createPost}>
            <p className={styles.blankLine}>Create Post</p>
          </a>
        </button>
        <b className={styles.initiativePosts}>Initiative Posts</b>
        <img className={styles.image102Icon} alt="" src="/image-102@2x.png" />
        <div className={styles.descriptionAboutCommunity1}>
          Description about community post Lorem ipsum dolor sit, amet
          consectetur adipisicing elit. Cumque similique eaque delectus unde
          commodi assumenda, rerum, et doloribus nobis qui eius voluptatum vitae
          suscipit quos quod? Description about community Lorem ipsum dolor sit,
          amet consectetur adipisicing elit. Cumque similique eaque delectus
          unde commodi assumenda, rerum, et doloribus nobis qui eius voluptatum
          vitae suscipit quos quod?
        </div>
        <div className={styles.frameChild2} />
        <img className={styles.lineIcon} alt="" src="/line-50.svg" />
        <div className={styles.frameChild3} />
        <img className={styles.image104Icon} alt="" src="/image-104@2x.png" />
        <img className={styles.image105Icon} alt="" src="/image-105@2x.png" />
        <div className={styles.nameOfPost}>Name of Post Owner</div>
        <img className={styles.image106Icon} alt="" src="/image-106@2x.png" />
        <div className={styles.rectangleDiv} />
        <img className={styles.frameChild4} alt="" src="/ellipse-20.svg" />
        <img className={styles.image51Icon1} alt="" src="/image-511@2x.png" />
        <div className={styles.addComment}>Add comment</div>
        <img className={styles.image108Icon} alt="" src="/image-108@2x.png" />
        <img className={styles.image109Icon} alt="" src="/image-108@2x.png" />
        <img className={styles.image110Icon} alt="" src="/image-108@2x.png" />
        <img className={styles.image111Icon} alt="" src="/image-108@2x.png" />
        <div className={styles.comments}>20 Comments</div>
        <img className={styles.image112Icon} alt="" src="/image-105@2x.png" />
        <div className={styles.nameOfPost1}>Name of Post Owner</div>
        <img className={styles.image103Icon} alt="" src="/image-103@2x.png" />
        <img className={styles.frameChild5} alt="" src="/ellipse-20.svg" />
        <img className={styles.image113Icon} alt="" src="/image-511@2x.png" />
        <div className={styles.descriptionAboutCommunity2}>
          Description about community post Lorem ipsum dolor sit, amet
          consectetur adipisicing elit. Cumque similique eaque delectus unde
          commodi assumenda, rerum, et doloribus nobis qui eius voluptatum vitae
          suscipit quos quod? Description about community Lorem ipsum dolor sit,
          amet consectetur adipisicing elit. Cumque similique eaque delectus
          unde commodi assumenda, rerum, et doloribus nobis qui eius voluptatum
          vitae suscipit quos quod?
        </div>
        <img className={styles.frameChild6} alt="" src="/line-50.svg" />
        <img className={styles.image114Icon} alt="" src="/image-104@2x.png" />
        <img className={styles.image116Icon} alt="" src="/image-106@2x.png" />
        <div className={styles.comments1}>20 Comments</div>
        <div className={styles.addComment1}>Add comment</div>
        <div className={styles.frameChild7} />
        <img className={styles.image117Icon} alt="" src="/image-108@2x.png" />
        <img className={styles.image118Icon} alt="" src="/image-108@2x.png" />
        <img className={styles.image119Icon} alt="" src="/image-108@2x.png" />
        <img className={styles.image120Icon} alt="" src="/image-108@2x.png" />
        <img className={styles.image121Icon} alt="" src="/image-108@2x.png" />
        <img className={styles.image122Icon} alt="" src="/image-108@2x.png" />
        <img className={styles.image123Icon} alt="" src="/image-108@2x.png" />
        <img className={styles.image124Icon} alt="" src="/image-108@2x.png" />
        <img className={styles.image125Icon} alt="" src="/image-108@2x.png" />
        <div className={styles.view55More}>view 55 more comments</div>
        <div className={styles.descriptionAboutCommunity3}>
          Description about community post Lorem ipsu
        </div>
        <div className={styles.tagsParent}>
          <b className={styles.tags}>Tags</b>
          <button className={styles.seeAllTags}>See all tags....</button>
          <div className={styles.rectangleContainer}>
            <div className={styles.groupChild1} />
            <div className={styles.medical}>
              <p className={styles.blankLine}>Medical</p>
              <p className={styles.blankLine}>&nbsp;</p>
            </div>
          </div>
          <div className={styles.groupDiv}>
            <div className={styles.groupChild2} />
            <div className={styles.education}>
              <p className={styles.blankLine}>Education</p>
            </div>
          </div>
          <div className={styles.rectangleParent1}>
            <div className={styles.groupChild3} />
            <div className={styles.sports}>
              <p className={styles.blankLine}>Sports</p>
            </div>
          </div>
          <div className={styles.rectangleParent2}>
            <div className={styles.groupChild4} />
            <div className={styles.animalAndPets}>Animal and Pets</div>
          </div>
        </div>
        <div className={styles.titleOfTheInitiativeWrapper}>
          <div className={styles.titleOfThe}>Title of the Initiative</div>
        </div>
        <div className={styles.helloDescriptionAbout}>
          Hello, Description about
        </div>
        <div
          className={styles.descriptionAboutCommunity4}
        >{`Description about community post Lorem ipsum Descriptio asflgkjsdflkmvldkcmv al fsa fg sdfgsdølfkgøsd fg sd fg sødflkvpsdokfg sdf . `}</div>
        <div className={styles.helloDescriptionAbout1}>
          Hello, Description about
        </div>
        <div className={styles.helloDescriptionAbout2}>
          Hello, Description about
        </div>
        <div className={styles.name}>Name</div>
        <div className={styles.name1}>Name</div>
        <div className={styles.name2}>Name</div>
        <div className={styles.name3}>Name</div>
        <div className={styles.helloDescriptionAbout3}>
          Hello, Description about
        </div>
        <div className={styles.name4}>Name</div>
        <div className={styles.helloDescriptionAbout4}>
          Hello, Description about
        </div>
        <div className={styles.descriptionAboutCommunity5}>
          Description about community post Lorem ipsu
        </div>
        <div className={styles.helloDescriptionAbout5}>
          Hello, Description about
        </div>
        <div className={styles.descriptionAboutCommunity6}>
          Description about community post Lorem ipsu
        </div>
        <div className={styles.descriptionAboutCommunity7}>
          Description about community post Lorem ipsu
        </div>
        <div className={styles.descriptionAboutCommunity8}>
          Description about community post Lorem ipsu
        </div>
        <div className={styles.helloDescriptionAbout6}>
          Hello, Description about
        </div>
        <div className={styles.name5}>Name</div>
        <div className={styles.name6}>Name</div>
        <div className={styles.name7}>Name</div>
        <div className={styles.name8}>Name</div>
        <div className={styles.name9}>Name</div>
        <div className={styles.name10}>Name</div>
        <div className={styles.name11}>Name</div>
        <div className={styles.name12}>Name</div>
        <div className={styles.image51Parent}>
          <img className={styles.image51Icon2} alt="" src="/image-51@2x.png" />
          <img className={styles.image61Icon} alt="" src="/image-61@2x.png" />
          <div className={styles.frameDiv} />
          <div className={styles.frameChild8} />
        </div>
        <img className={styles.image95Icon} alt="" src="/image-95@2x.png" />
        <div className={styles.initiativeOwner}>Initiative owner</div>
        <div className={styles.supportedBy}>Supported By</div>
        <div className={styles.image65Parent}>
          <img className={styles.image65Icon} alt="" src="/image-65@2x.png" />
          <div className={styles.nameOfUser1}>Name of User</div>
          <div className={styles.donated20}>
            <span>{`Donated   `}</span>
            <span className={styles.span1}>£20</span>
          </div>
        </div>
        <div className={styles.image65Group}>
          <img className={styles.image65Icon} alt="" src="/image-65@2x.png" />
          <div className={styles.nameOfUser1}>Name of User</div>
          <div className={styles.donated20}>
            <span>{`Donated   `}</span>
            <span className={styles.span1}>£20</span>
          </div>
        </div>
        <div className={styles.image65Container}>
          <img className={styles.image65Icon} alt="" src="/image-65@2x.png" />
          <div className={styles.nameOfUser1}>Name of User</div>
          <div className={styles.donated20}>
            <span>{`Donated   `}</span>
            <span className={styles.span1}>£20</span>
          </div>
        </div>
        <div className={styles.image65Parent1}>
          <img className={styles.image65Icon} alt="" src="/image-65@2x.png" />
          <div className={styles.nameOfUser1}>Name of User</div>
          <div className={styles.donated20}>
            <span>{`Donated   `}</span>
            <span className={styles.span1}>£20</span>
          </div>
        </div>
      </div>
      <div className={styles.copyrightAndInformationAbouWrapper}>
        <div className={styles.copyrightAndInformation}>
          @copyright and information about the TAKK
        </div>
      </div>
      <div className={styles.editcreateCommunityPage}>
        <div className={styles.takkLogoYellow2Group}>
          <img
            className={styles.takkLogoYellow21}
            alt=""
            src="/takk-logo--yellow-22@2x.png"
          />
          <div className={styles.frameWrapper}>
            <div className={styles.frameChild} />
          </div>
          <div className={styles.searchParent}>
            <div className={styles.search}>Search</div>
            <img className={styles.image6Icon} alt="" src="/image-6@2x.png" />
          </div>
          <div className={styles.nameOfUser5}>Name of User</div>
          <img className={styles.frameItem} alt="" src="/ellipse-4@2x.png" />
          <div className={styles.ellipseGroup}>
            <img className={styles.groupChild} alt="" src="/ellipse-51.svg" />
            <div className={styles.div}>3</div>
          </div>
          <div className={styles.getFundraisingParent}>
            <div className={styles.getFundraising1}>Get Fundraising</div>
            <div className={styles.donate1}>Donate</div>
            <div className={styles.community1}>Community</div>
          </div>
          <div className={styles.frameInner}>
            <div className={styles.frameChild11} />
          </div>
          <div className={styles.frameChild12} />
        </div>
        <div className={styles.frameParent}>
          <div className={styles.aboutYourCommunityParent}>
            <div className={styles.aboutYourCommunity}>
              About Your Community
            </div>
            <div className={styles.flexibleDuration}>Flexible Duration</div>
            <div className={styles.ourPagesHave}>
              Our pages have an initial duration of 120 days, but you can extend
              it up to 1 year if you prefer.
            </div>
            <div className={styles.fees}>Fees</div>
            <div className={styles.someWords}>
              <p className={styles.blankLine}>Some words</p>
            </div>
            <div className={styles.payments}>Payments</div>
            <div className={styles.youWillReceive}>
              You will receive the money you raise even if you don't hit your
              target. Your funds will be sent to the bank account of your choice
              within 4-6 business days after your Initative closes
            </div>
          </div>
          <div className={styles.deleteYourCommunity}>
            Delete Your Community
          </div>
          <div className={styles.rectangleParent3}>
            <div className={styles.groupChild6} />
            <div className={styles.deleteTheService}>Delete the Service</div>
          </div>
          <div className={styles.changeYourAlert}>Change your Alert</div>
          <div className={styles.frameGroup}>
            <div className={styles.frameChild13} />
            <div className={styles.off}>OFF</div>
          </div>
          <div className={styles.onWrapper}>
            <div className={styles.on}>ON</div>
          </div>
          <img className={styles.frameChild14} alt="" src="/line-32.svg" />
          <img className={styles.frameChild15} alt="" src="/line-33.svg" />
          <div className={styles.youWillGet}>
            You will get Alert every time when someone follow your community
          </div>
        </div>
        <div className={styles.editcreateCommunityPageChild} />
        <div className={styles.copyrightAndInformationAbouContainer}>
          <div className={styles.copyrightAndInformation}>
            @copyright and information about the TAKK
          </div>
        </div>
        <div className={styles.summaryOfServiceParent}>
          <div className={styles.summaryOfServiceContainer}>
            <p className={styles.blankLine}>Summary of Service</p>
            <p className={styles.blankLine}>&nbsp;</p>
          </div>
          <b className={styles.titleOfThe1}>Title of the Community</b>
          <div className={styles.frameChild16} />
          <b className={styles.b1}>35</b>
          <div className={styles.headlineOfCommunity}>
            Headline of Community by owner
          </div>
          <b className={styles.serviceOwner}>Service Owner</b>
          <img className={styles.image83Icon} alt="" src="/image-83@2x.png" />
          <b className={styles.profilePicture}>Profile Picture</b>
          <b className={styles.firstName}>First Name</b>
          <div className={styles.frameChild17} />
          <b className={styles.lastName}>Last Name</b>
          <div className={styles.frameChild18} />
          <b className={styles.yourMessage}>Your Message</b>
          <div className={styles.frameChild19} />
          <b className={styles.b2}>200</b>
          <div className={styles.thisMessageWill}>
            This message will displayed just after the Headline of the Service
          </div>
          <b className={styles.coverImagesOf}>Cover Images of Community</b>
          <img className={styles.image82Icon} alt="" src="/image-82@2x.png" />
          <img className={styles.image51Icon3} alt="" src="/image-51@2x.png" />
          <img className={styles.image62Icon1} alt="" src="/image-62@2x.png" />
          <img className={styles.image63Icon1} alt="" src="/image-63@2x.png" />
          <img className={styles.frameChild20} alt="" src="/line-17.svg" />
          <b className={styles.descriptionOfTheContainer}>
            <p className={styles.blankLine}>Description of the Community</p>
          </b>
          <div className={styles.frameChild21} />
          <b className={styles.b3}>200</b>
          <div className={styles.frameChild22} />
          <div className={styles.rectangleParent4}>
            <div className={styles.groupChild7} />
            <div className={styles.deleteTheService}>Cancel</div>
          </div>
          <div className={styles.rectangleParent5}>
            <div className={styles.groupItem} />
            <div className={styles.deleteTheService}>Save and Continue</div>
          </div>
          <img className={styles.frameChild23} alt="" src="/ellipse-14.svg" />
          <div className={styles.div2}>?</div>
          <img className={styles.frameChild24} alt="" src="/ellipse-14.svg" />
          <div className={styles.div3}>?</div>
          <img className={styles.frameChild25} alt="" src="/ellipse-14.svg" />
          <div className={styles.div4}>?</div>
          <img className={styles.frameChild26} alt="" src="/ellipse-14.svg" />
          <div className={styles.div5}>?</div>
          <img className={styles.frameChild27} alt="" src="/ellipse-14.svg" />
          <div className={styles.div6}>?</div>
          <img className={styles.frameChild28} alt="" src="/ellipse-14.svg" />
          <div className={styles.div7}>?</div>
          <div className={styles.pleaseEnterThis}>
            Please enter this in the format XYZ
          </div>
          <div className={styles.frameChild29} />
          <div className={styles.thisIsAn}>
            This is an example of a tool tip. It shows when the question mark is
            hovered over or tapped and gives speci
          </div>
        </div>
      </div>
      <div className={styles.communitiesParent}>
        <b className={styles.communities}>Communities</b>
        <div className={styles.frameChild30} />
        <div className={styles.seeAllCommunity}>
          See all community initiatives
        </div>
        <div className={styles.image7Parent}>
          <img className={styles.image7Icon} alt="" src="/image-7@2x.png" />
          <div className={styles.overviewOfTheInitiativeForParent}>
            <div className={styles.overviewOfThe}>
              Overview of the Initiative for the users to get an idea
            </div>
            <a className={styles.rectangleWrapper}>
              <div className={styles.groupChild9} />
            </a>
            <a className={styles.findOutMore}>Find out more</a>
            <b className={styles.titleOfThe2}>Title of the Community</b>
          </div>
        </div>
        <div className={styles.image7Group}>
          <img className={styles.image7Icon} alt="" src="/image-71@2x.png" />
          <div className={styles.overviewOfTheInitiativeForGroup}>
            <div className={styles.overviewOfThe}>
              Overview of the Initiative for the users to get an idea
            </div>
            <a className={styles.rectangleWrapper}>
              <div className={styles.groupChild9} />
            </a>
            <a className={styles.findOutMore1}>Find out more</a>
            <b className={styles.titleOfThe3}>Title of the Community</b>
          </div>
        </div>
      </div>
      <div className={styles.showMore}>Show more</div>
    </div>
  );
};

export default ViewInitiativePage;
